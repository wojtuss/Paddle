...
void CPUQuantizeSquashPass::ReshapeTransposeScaleSquash(
    Graph* graph) const{
    GraphPatternDetector gpd;
    patterns::ReshapeTransposeScale reshape_transpose_scale_pattern{gpd.mutable_pattern(), "reshape_transpose_scale"};  
...
void CPUQuantizeSquashPass::ConvDequantSquash(Graph* graph) const {
  ...
  void CPUQuantizeSquashPass::FcDequantSquash(Graph* graph) const{
    GraphPatternDetector gpd;
    patterns::FcDequant fc_dequant_pattern{gpd.mutable_pattern(),
                                             "fc_dequant"};
    fc_dequant_pattern();
    ...
    auto handler = [&](const GraphPatternDetector::subgraph_t& subgraph,
      ...
      // if fc with force_fp32_output
      if (fc_out->outputs.size() == 1) {
        fc_op->Op()->SetAttr("force_fp32_output", true);
        fc_op->Op()->SetOutput("Out", std::vector<std::string>({dequant_out->Name()}));
