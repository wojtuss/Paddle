...
void SetOp(ProgramDesc* prog, const std::string& type, const std::string& name,
  ...
  if (type == "conv2d") {
    ...
    op->SetInput("Input", {inputs[0]});
    PADDLE_ENFORCE_EQ(inputs.size(), 2,"The fc inputs should have input and weights.");
...
// build fc_dequant
ProgramDesc BuildFcDequantConcatProgramDesc(bool use_mkldnn, float scale_out, float scale){
  ProgramDesc prog;
  for (auto& v : variable_names) {
    prog.MutableBlock(0)->Var(v);
  }
