...
// Conv with batch norm on one of the output branch:
// conv + (batch_norm, ...)
// named nodes:
// conv_input, conv_filter, conv, conv_output, bn
struct ConvBifBN : public PatternBase {
  ConvBifBN(PDPattern* pattern, const std::string& name_scope)
      : PatternBase(pattern, name_scope, "conv_bif_bn") {}
