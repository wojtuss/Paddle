...
PDNode *patterns::ReshapeTransposeScale::operator()() {
  ...
  auto reshape_op = pattern->NewNode(reshape_op_repr())->assert_is_op("reshape2");
  auto transpose_op = pattern->NewNode(transpose_op_repr())->assert_is_op("transpose2");
  auto scale_op = pattern->NewNode(scale_op_repr())->assert_is_op("scale");

  auto reshape_out = pattern->NewNode(reshape_out_repr())->AsOutput()->assert_is_op_output("reshape2", "Out");
